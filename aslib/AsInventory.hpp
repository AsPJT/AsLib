...
namespace AsLib
{
	...
	struct Item {
		...
		std::size_t stack_max{};
		std::string name{};
		Item() = default;
		Item(const std::size_t type_, AsTexture& t_, const std::size_t stack_, const char* const name_) :type(type_), texture(&t_), stack_max(stack_), name(name_) {}
		Item(AsTexture* const t_, const std::size_t stack_, const char* const name_) :type(0), texture(t_), stack_max(stack_), name(name_) {}
		Item(const std::size_t type_, AsTexture& t_, const char* const name_) :type(type_), texture(&t_), stack_max(1), name(name_) {}
	...
	struct UniqueItem {
		...
		std::size_t stack{};
		UniqueItem() = default;
		constexpr UniqueItem(const std::size_t item_, const std::size_t stack_) :item(item_), stack(stack_) {}
	...
	struct ShareItem {
		...
		ShareItem() = default;
		ShareItem(const std::size_t max_) {
			const std::size_t before_size{ unique.size() };
			unique.resize(max_);
			for (std::size_t i{ before_size }; i < max_; ++i) {
				unique[i].item = 0;
				unique[i].stack = 0;
			}
		}

		ShareItem& setSize(const std::size_t max_) noexcept {
			const std::size_t before_size{ unique.size() };
			unique.resize(max_);
			for (std::size_t i{ before_size }; i < max_; ++i) {
				unique[i].item = 0;
		...
		ShareItem& addSize(const std::int32_t add_) noexcept {
			const std::size_t before_size{ unique.size() };
			const std::size_t total_size{ ((std::int32_t(unique.size()) + add_) < 0) ? 0 : std::size_t((before_size + add_)) };
			unique.resize(total_size);
			for (std::size_t i{ before_size }; i < total_size; ++i) {
				unique[i].item = 0;
				unique[i].stack = 0;
			}
			return *this;
...
#endif
		std::size_t size() const noexcept { return unique.size(); }
		ShareItem& push(const std::size_t id_, const std::size_t item_, const std::size_t stack_ = 1) noexcept {
			unique[id_].item = item_;
			unique[id_].stack = stack_;
			return *this;
		}
		ShareItem& push(const std::size_t id_, const UniqueItem& item_) noexcept {
			unique[id_].item = item_.item;
			unique[id_].stack = item_.stack;
			return *this;
		}
		//新しいアイテムを追加する
		UniqueItem add(const std::size_t item_, const std::size_t stack_,const std::size_t max_) noexcept {
			std::size_t add_stack{ stack_ };
			std::size_t s{};
			for (std::size_t i{}; i < unique.size(); ++i) {
		...
		UniqueItem add(const UniqueItem& item_, const std::size_t max_) noexcept { return add(item_.item, item_.stack, max_); }

		//中身を消去する
		UniqueItem clear(const std::size_t id_) noexcept {
	...
	struct Inventory {
		...
		std::size_t select_frame{};

		Inventory(AsTexture& frame_, AsTexture& select_frame_texture, AsFont& font_, ShareItem& share_, const PosA4& pos_, const std::size_t num_frame_, const std::size_t select_frame_)
		...
		Inventory& selectAdd(const std::int32_t add_) noexcept {
		...
		Inventory& draw(const std::vector<Item>& item_) noexcept {
			...
			draw_pos.x = pos.x - (std::int32_t(num_frame * pos.w) - pos.w) / 2;
			for (std::size_t i{}; i < num_frame; ++i) {
		...
		Inventory& addSize(const std::int32_t add_) noexcept {
			if (share_item == nullptr) return *this;
			share_item->addSize(add_);
			num_frame = ((std::int32_t(num_frame) + add_) < 0) ? 0 : std::size_t((num_frame + add_));
			return *this;
		}
