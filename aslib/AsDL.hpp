...
//#if defined(USING_DXLIB)
	namespace test
	{
		...
		inline bool ProcessMessageControl(const bool is_ = true, const bool on_ = false) { static bool is_message = false; if (on_) is_message = is_; return is_message; }
		inline int ProcessMessage() { return ProcessMessageControl(); }
		...
		int ScreenFlip() { if (asLoop() == false) { ProcessMessageControl(true, true); return -1; } return 0; }
		...
		int GetMousePoint(int* const x_, int* const y_) { const Pos2 p = mousePos(); *x_ = int(p.x); *y_ = int(p.y); return 0; }
		int GetMousePoint(int& x_, int& y_) { const Pos2 p = mousePos(); x_ = int(p.x); y_ = int(p.y); return 0; }
		int GetMousePoint(Pos2& p_) { p_ = mousePos(); return 0; }
